...
const TARGET_STREAK = 10 // 连续答对10题通关
...
export default function ChallengeMode({ words, onComplete, onBack }: ChallengeModeProps) {
  ...
  const [gameState, setGameState] = useState<'start' | 'playing' | 'success' | 'failed'>('start')
        ...
        <div className="card">
          ...
          <div className="bg-gradient-to-r from-orange-50 to-red-50 rounded-xl p-4 mb-6">
            <p className="text-sm text-orange-800">
              ⚠️ <span className="font-bold">紧张刺激！</span>
            </p>
        ...
        <div className="card animate-bounce-in">
          ...
          <div className="bg-green-50 rounded-xl p-4 mb-6">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <p className="text-sm text-gray-600">连续答对</p>
                <p className="text-2xl font-bold text-green-600">{streak} 题</p>
              </div>
              <div>
                <p className="text-sm text-gray-600">评价</p>
                <p className="text-2xl font-bold text-yellow-600">S 级</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex gap-3">
            <button onClick={onBack} className="flex-1 bg-gray-100 text-gray-700 font-bold py-4 px-6 rounded-xl transition-all hover:bg-gray-200">
              返回
            </button>
            <button onClick={startGame} className="flex-1 bg-gradient-to-r from-green-500 to-emerald-600 text-white font-bold py-4 px-6 rounded-xl transition-all hover:shadow-lg">
              再来一次 🎮
            </button>
          </div>
        </div>
      ...
      <div className="max-w-md mx-auto text-center">
        <div className="card animate-shake">
          ...
          <div className="bg-red-50 rounded-xl p-4 mb-6">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <p className="text-sm text-gray-600">本次连击</p>
                <p className="text-2xl font-bold text-red-600">{streak} 题</p>
              </div>
              <div>
                <p className="text-sm text-gray-600">最佳连击</p>
          ...
          <div className="bg-blue-50 rounded-xl p-4 mb-6">
            ...
            <p className="text-sm text-blue-700 mt-2">
              还需要连续答对 <span className="font-bold">{TARGET_STREAK - streak}</span> 题
            </p>
          </div>
          ...
          <div className="flex gap-3">
            ...
            <button onClick={startGame} className="flex-1 bg-gradient-to-r from-orange-500 to-red-500 text-white font-bold py-4 px-6 rounded-xl transition-all hover:shadow-lg">
              再来一次 💪
            </button>
          </div>
        </div>
    ...
    <div ref={containerRef} className="max-w-md mx-auto">
      ...
      <div className="flex justify-between items-center mb-4">
        <div className="flex items-center gap-2">
          ...
          <span className={`text-2xl font-bold ${streak >= 7 ? 'text-orange-500 animate-pulse' : 'text-blue-600'}`}>
            {streak}
          </span>
        ...
        <div className="flex items-center gap-2">
          {streak >= 3 && (
            <span className="text-sm bg-orange-100 text-orange-700 px-2 py-1 rounded-full animate-bounce">
              🔥 连击中！
            </span>
          )}
        </div>
      ...
      <div className="mb-6">
        ...
        {streak >= 8 && (
          <p className="text-center text-sm text-orange-600 font-bold mt-1 animate-pulse">
            🔥 就差 {TARGET_STREAK - streak} 题了！
          </p>
        )}
      </div>
      ...
      <div className={`card mb-6 transition-all duration-300 ${
        shakeWrong ? 'animate-shake bg-red-50 border-2 border-red-300' : ''
      } ${
        showResult 
      ...
      {/* 结果提示 */}
      {showResult && (
        <div className={`mt-4 p-4 rounded-xl text-center animate-bounce-in ${
          selectedAnswer === currentWord?.word.meaning
          ...
          {selectedAnswer === currentWord?.word.meaning ? (
            <p className="font-bold">
              ✅ 太棒了！{streak >= TARGET_STREAK - 1 ? '🎉 即将通关！' : `继续！还差 ${TARGET_STREAK - streak} 题`}
            </p>
          ) : (
            <div>
              <p className="font-bold mb-1">❌ 答错了！</p>
              <p className="text-sm">正确答案：{currentWord?.word.meaning}</p>
            </div>
          )}
        </div>
      )}
